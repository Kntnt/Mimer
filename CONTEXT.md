...
What the judgment rules flag as unsafe to make global without asking: material carrying an explicit confidentiality signal — a stated confidentiality, an NDA or non-disclosure term, a "don't share this" marker. The classifier is deliberately tight (ask-rarely): a plain client identity — a name, an email, a phone number — is not sensitive on its own and may be promoted to global scope like any other client-neutral fact, unless the user has explicitly flagged it confidential. The axis is "is this confidential?", not "is this about a client?". A sensitive fact is never promoted to global scope without explicit user consent, asked at the next session start and waiting project-bound until answered — and resolved in the moment when distillation is triggered manually ("distill now") (ADR 0027).
